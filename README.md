...
## v4.10 — SEO leadership batch
- `tonight.html` / `this-week.html`: intent answer pages regenerated by the Action every run (content baked into HTML + FAQ schema).
- `news/<slug>-results.html` + `news.html`: automatic post-launch recap articles (NewsArticle schema) — a fresh-content stream for Top Stories/Discover.
- Dynamic homepage `<title>`/description: the Action writes the actual next-launch name+time into the SERP snippet each run.
- `img/next.jpg`: Action self-hosts the next launch's hero photo (fast LCP); `<meta name="rlt-hero-id">` tells the app when to use it.
- 14 city viewing-guide pages (`see-launch-from-*.html`), `about.html` (E-E-A-T), FAQPage schema on the glossary, BreadcrumbList + VideoObject + related-launch links on mission pages, Organization schema on the homepage.
- IndexNow: key file `a7c4f9e2ab5d84e21b6f3a8c9d0e4f71.txt` + Action ping after each data commit (Bing/DuckDuckGo/Yandex index new pages in minutes).
- Maintenance note: regenerate the 16 provider/rocket/location pages with `regen-presets.py` (regex-based — safe with the dynamic homepage title).
